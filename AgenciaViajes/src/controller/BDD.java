...
public class BDD {
	...
	protected CachedRowSet consultaSQL(String sql) {
		Connection cnx = new SqLiteConexion().getConection();
...
//	}
	...
	protected Integer ejecutaSQL(String sql) {
		Connection cnx = new SqLiteConexion().getConection();
		Integer resp = ejecutaSQL(cnx, sql);
		try {
			cnx.close();
		} catch (SQLException e) {
			e.printStackTrace();
	 ...
	 */
	protected Integer ejecutaSQL(Connection cnx, String sql) {
		Integer respuesta = null;
		if (cnx!=null) {
			System.out.println(sql);
			try {
				Statement sentencia = cnx.createStatement();
				//devuelve 0 si no se ha echo nada, y N el numero de afectadas
				respuesta = sentencia.executeUpdate(sql);
				respuesta = respuesta==0?-1:0;
			} catch (SQLException e) {
				e.printStackTrace();
			}
		}
