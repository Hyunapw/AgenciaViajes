...
public class GrupoAsientosOcupados extends JPanel {
	...
	public GrupoAsientosOcupados(Dimension dimension) {
		if (dimension!=null) {
	...
	public GrupoAsientosOcupados(ArrayList<Integer> list, int columns, int rows) {
		if (list!=null && rows>=0 && columns>=0) {
			this.filas = rows;
			this.columnas = columns;
			inicia();
			ponerGrupoOcupados(list);
		}
	}

	public GrupoAsientosOcupados(int columns, int rows) {
		if (rows>=0 && columns>=0) {
			this.columnas = columns;
			this.filas = rows;
			inicia();
...
//	private void ponerGrupoOcupados(ArrayList<Asiento> lista) {
//		for (Asiento asientoDato : lista) {
//			
//			
//			
//			AsientoBtn asientoBtn = new AsientoBtn(asientoDato);
//			add(asientoBtn);
//		}
//	}
	
	private void ponerGrupoOcupados(ArrayList<Integer> listaOcupados) {
		int contador = 1;
		boolean ocupado = false;
		for (int i = 1; i < filas+1; i++) {
			for (int j = 1; j < columnas+1; j++) {
				for (Integer plazaOcupada : listaOcupados) {
					if (contador == plazaOcupada) {
						ocupado = true;
					}
				}
				BtAsiento asiento = new BtAsiento(contador, "F" + i + "C" + j, 1, ocupado);
				add(asiento);
				contador++;
			}
		}
